...
export default function AdminElections() {
  ...
  const downloadPDF = async (id, title) => {
    try {
      toast.info(`Generating executive PDF result report for "${title}"...`);
      const response = await api.get(`/reports/election/${id}/pdf`, { responseType: 'blob' });
      
      const blob = new Blob([response.data], { type: 'application/pdf' });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `votechain-election-${id}-results.pdf`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      toast.success('PDF report fetched successfully!');
    } catch (err) {
      console.error('PDF fetch error:', err);
      toast.error('Failed to compile PDF result document.');
    }
  };

  const downloadCSV = async (id, title) => {
    try {
      toast.info(`Compiling CSV data table for "${title}"...`);
      const response = await api.get(`/reports/election/${id}/csv`, { responseType: 'blob' });
      
      const blob = new Blob([response.data], { type: 'text/csv' });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `votechain-results-${id}.csv`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      toast.success('CSV table downloaded successfully!');
    } catch (err) {
      console.error('CSV compilation error:', err);
      toast.error('Failed to generate CSV data resource.');
    }
  };
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-[10px] font-bold uppercase tracking-wider text-slate-500">
                ...
                <th className="px-6 py-4 text-center">Export Results</th>
              ...
              {elections.map((el) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-center" onClick={(e) => e.stopPropagation()}>
                    {el.status === 'CLOSED' || el.status === 'RESULTS_PUBLISHED' ? (
                      <div className="flex items-center justify-center gap-2">
                        <button
                          onClick={() => downloadPDF(el.id, el.title)}
                          title="Download PDF Executive Report"
                          className="bg-pdf-color bg-rose-50 border border-rose-200 hover:bg-rose-100 text-rose-700 px-2.5 py-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1 shadow-sm hover:shadow-md cursor-pointer"
                        >
                          <FileText className="h-3.5 w-3.5" /> PDF
                        </button>
                        <button
                          onClick={() => downloadCSV(el.id, el.title)}
                          title="Download CSV Candidate Tally Sheet"
                          className="bg-amber-50 border border-amber-200 hover:bg-amber-100 text-amber-700 px-2.5 py-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1 shadow-sm hover:shadow-md cursor-pointer"
                        >
                          <Download className="h-3.5 w-3.5" /> CSV
                        </button>
                      </div>
                    ) : (
                      <span className="text-xs text-slate-400 italic">Pre-closing lock</span>
                    )}
                  </td>
