...
/**
 * Creates email transport based on SMTP settings from the environment variables.
 * Automatically falls back to standard log outputs if SMTP credentials are not configured.
 ...
 * @returns {Promise<boolean>} True if sent successfully, or simulating successful delivery.
 */
...
export async function sendElectionOpenEmail(emails, electionTitle) {
  ...
  try {
    // Send to recipients individually to respect address anonymity
    await Promise.all(
      emails.map((recipient) =>
        transporter.sendMail({
          from: `"Election Commission of Bangladesh" <${process.env.EMAIL_USER}>`,
          to: recipient,
          subject: subject,
          html: htmlContent,
        })
