...
export async function generateElectionResultPDF(electionId, prisma) {
  ...
  const candidatesWithVotes = candidates.map((cand) => {
    ...
    return {
      id: cand.id,
      name: cand.user.name,
      partyName: cand.party ? cand.party.name : 'Independent',
      partyAbbr: cand.party ? cand.party.abbreviation : 'IND',
  ...
  const hoverHighlight = rgb(230/255, 245/255, 238/255);
  ...
  const drawPageFooter = (page, pageNum) => {
    ...
    page.drawText('Generated by OneID Bangladesh | CSE412 Project | East West University', {
      x: margin,
      y: 40,
      size: 8,
      font: fontRegular,
  ...
  // COVER PAGE
  {
    ...
    page.drawText('Digital Voting Infrastructure & Secure Ledger Ecosystem', {
      x: margin,
      y: pageHeight - 125,
      size: 14,
      font: fontRegular,
    ...
    page.drawText('This dossier compiles authorized tallies generated dynamically by OneID.', {
      x: margin + 20,
      y: 195,
      size: 9,
      font: fontRegular,
  ...
  // BLOCKCHAIN Security Auditory Page
  {
    const page = pdfDoc.addPage([pageWidth, pageHeight]);
    ...
    const drawChainLink = (nameText, hashText) => {
      page.drawRectangle({
        x: blockX,
        y: blockY,
        width: blockW,
    ...
    drawChainLink('GENESIS BLOCK', '00000000');
